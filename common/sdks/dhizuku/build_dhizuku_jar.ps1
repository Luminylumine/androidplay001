...
$src   = Join-Path $root ("clawphone\libs\" + $(if ($args.Count -gt 0) { $args[0] } else { "dz24" }))
...
# 5. install into libs
Copy-Item $jar (Join-Path $root "clawphone\libs\dhizuku-api\classes.jar") -Force
Write-Host "[dz] installed:" (Get-Item (Join-Path $root "clawphone\libs\dhizuku-api\classes.jar")).Length "bytes"
