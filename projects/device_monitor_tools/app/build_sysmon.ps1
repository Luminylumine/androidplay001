# SysMon local build: aapt2 -> javac -> d8 -> apksigner
# Usage: powershell -NoProfile -ExecutionPolicy Bypass -File build_sysmon.ps1
...
Write-Host "[sysmon] repo root resolved: $root"

# Project lives at repo/projects/device_monitor_tools/app (PSScriptRoot is app/)
$proj  = $PSScriptRoot
...
function Log($m) { Write-Host "[sysmon] $m" }
...
$srcs = @(Get-ChildItem -Recurse -Path (Join-Path $proj "src") -Filter *.java | ForEach-Object { # SysMon local build: aapt2 -> javac -> d8 -> apksigner
# Usage: powershell -NoProfile -ExecutionPolicy Bypass -File build_sysmon.ps1
$ErrorActionPreference = "Stop"
...
# 3. javac (classpath includes Shizuku/Dhizuku api jars + aidl stub)
Log "javac"
...
$libs = @(
    (Join-Path $proj "libs\shizuku-api\classes.jar"),
    (Join-Path $proj "libs\shizuku-provider\classes.jar"),
    (Join-Path $proj "libs\shizuku-aidl\classes.jar"),
    (Join-Path $proj "libs\dhizuku-api\classes.jar")
)
...
Write-Host ("[sysmon] classes.dex = {0} bytes, unsigned.apk = {1} bytes" -f (Get-Item (Join-Path $build "classes.dex")).Length, (Get-Item $apk).Length)

# 6. sign
Log "sign"
$ks = Join-Path $proj "sysmon.keystore"
if (-not (Test-Path $ks)) {
    & "$java\keytool.exe" -genkeypair -v -keystore $ks -alias sysmon `
        -keyalg RSA -keysize 2048 -validity 10000 `
        -storepass sysmon123 -keypass sysmon123 `
        -dname "CN=SysMon, OU=Dev, O=SysMon, L=NA, ST=NA, C=CN" | Out-Null
}
$final = Join-Path $build "SysMon-v1.apk"
cmd /c "`"$java\java.exe`" -cp `"$bt\lib\apksigner.jar`" com.android.apksigner.ApkSignerTool sign --ks `"$ks`" --ks-pass pass:sysmon123 --key-pass pass:sysmon123 --ks-key-alias sysmon --out `"$final`" `"$apk`""
...
.FullName })
$srcs += (Get-ChildItem -Recurse -Path $gen -Filter *.java | ForEach-Object { # SysMon local build: aapt2 -> javac -> d8 -> apksigner
# Usage: powershell -NoProfile -ExecutionPolicy Bypass -File build_sysmon.ps1
$ErrorActionPreference = "Stop"
